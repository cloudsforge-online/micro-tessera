...
| --- | --- | --- |
| `OUTBOX_SIGNING_SECRET` | — | **required, ≥24 chars, a single key.** Signs what this service emits. It stays one value: signing with two at once would double every subscriber's verification work and leave nobody able to say which key an event was signed with (`src/env.ts:273`) |
| `INBOUND_SIGNING_SECRET` | — | **required, ≥24 chars — and a LIST, comma-separated, newest first.** Every key an inbound delivery may have been signed with. A list rather than a value because the estate's outbox secret is one key shared by 24 services, and swapping it partitions delivery: whichever end moves first has everything between them refused until the other catches up, and the failure reads as a secret mismatch rather than as a deploy ordering problem. So the rotation is a window — add the new key at the front, restart, move the producers, then drop the old one. One key is simply a list of one, so nothing about a current deployment changes. Every entry faces the same length and placeholder bar, and a repeat is refused: a duplicate makes "which key verified this" ambiguous, and that answer is how you know the rotation finished. A delivery verifying against anything but the first key is logged with its `keyIndex` (`src/env.ts:274`, parser at `:113-133`, use at `src/inbound.ts:81`) |
